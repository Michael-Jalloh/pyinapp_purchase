...
import requests
import jwt
from google_purchase_verifier.responses import GoggleResponse, GogglePurrchaseResponse, GoogleVerificationError


class GogglePurchaseProductVerifier(object):
    ...
    def verify_purchase(self, package_name: str, product_id: str, purchase_token: str, expiration: int =120) -> GogglePurrchaseResponse:
        claim_start = time.time()
        self.claims["exp"] = claim_start + expiration
        self.claims["iat"] = claim_start

        token = jwt.encode(self.claims, self.key, header=self.header, algorithm="RS256")
        params = {
        ...
        if response.status_code == 200:
            return GogglePurrchaseResponse(response.json())
        
        raise GoogleVerificationError(response.json())

    def consume_purchase(self, package_name: str, product_id: str, purchase_token: str, expiration: int = 120) -> GoggleResponse:
        claim_start = time.time()
        self.claims["exp"] = claim_start + expiration
        self.claims["iat"] = claim_start
        ...
        if response.status_code == 200:
            return GoggleResponse({"message":"purchase consumed"})
        
        raise GoogleVerificationError(response.json())
    
    def acknowledge_purchase(self, package_name: str, product_id: str, purchase_token: str, payload: str = "", expiration: int = 120) -> GoggleResponse:
        claim_start = time.time()
        self.claims["exp"] = claim_start + expiration
        self.claims["iat"] = claim_start
        ...
        response = requests.post(url=url, headers=headers, data={"developerPayload":payload})
